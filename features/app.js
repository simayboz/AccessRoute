(function () {
    ...
    async function resizeImage(base64Str, maxWidth = 800) {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                // Groq tam base64 istiyor (data:image/jpeg;base64,... dahil)
    ...
    function renderGiris() {
        ...
        return `<div class="flex flex-col items-center justify-center h-screen space-y-10 text-center px-10 animate-fade-in">
            ...
            <div class="w-full max-w-sm space-y-4">
                ...
                ${!hasKey ? `<input type="password" id="apiKeyInput" placeholder="Groq API Key (gsk_...)" class="w-full bg-slate-900 border border-slate-800 rounded-2xl py-5 px-6 text-emerald-400">` : `<div class="text-emerald-400 text-sm font-medium">✅ Groq API Hazır</div>`}
    ...
    async function runAI() {
        ...
        try {
            ...
            const prompt = `Sen bir kampüs erişilebilirlik uzmanısın. Kullanıcı Profili: ${state.userProfile}. Fotoğraftaki fiziksel engelleri kısa ve net bir dille analiz et.`;
            ...
            if (data.error) {
                state.geminiResult = `❌ Groq Hatası: ${data.error.message}`;
                ...
                state.geminiResult = `<div class="space-y-1 text-left">${resultText}</div>`;
            } else {
                state.geminiResult = "⚠️ Analiz yapılamadı.";
            }
        } catch (error) {
